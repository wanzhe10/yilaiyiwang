$(function() {
    ...
    $('.progressBar').find('li').eq(1).addClass('libg');
    $('.progressBar').find('li').eq(2).addClass('libg');
    $('.progressBar').find('li').eq(3).addClass('libg');
    /* 点击修改按钮收起显示 */
    // 患者基本信息essential_btn
    $('#cut').click(function() {
 ...
 if (recipientsArr.length == 0) {
    ...
    for (var i = 0; i < data.orderDoctorsList.length; i++) {
        goalHtml += '<pre class="report">' + data.orderDoctorsList[i].name + ':<br />' + data.orderDoctorsList[i].feedBack + '</pre>'
    }
    ...
    for (var i = 0; i < recipientsArr.length; i++) {
        ...
        _fees += '<td>'
        if (data.orderFormBean.orderTypes == 0) {
            _fees += '<input type="text" name="' + recipientsArr[i].orderDoctorId + '" value="' + recipientsArr[i].price + '" class="fees_input gai">'
        } else {
            _fees += '<input type="text" name="' + recipientsArr[i].orderDoctorId + '" value="' + recipientsArr[i].price + '" class="fees_input gai">'
        }
        _fees += '</td></tr>'
    ...
    $('.countInput').val(data.orderFormBean.money);

    $('.fees_table').find('.fees_input').blur(function() {
        var countFees = 0;
        $(this).val(Math.round($(this).val() * 100) / 100);
        if (isNaN($(this).val())) {
            $(this).val(0)
        }
        for (var i = 0; i < $('.fees_table').find('.fees_input').length; i++) {
            countFees += Math.round($('.fees_table').find('.fees_input').eq(i).val() * 100);
        }
        $('.countInput').val(countFees / 100);
    })
    var oldCountFees = 0; // 原总价
    var discount = 1; // 折率
    var docCount = 0; // 所有医生总价
    $('.countInput').focus(function() {
        oldCountFees = Math.round($(this).val() * 100)
    }).blur(function() {
        $(this).val(Math.round($(this).val() * 100) / 100);
        if (isNaN($(this).val())) {
            $(this).val(0)
        }
        discount = Math.round($(this).val() * 100) / oldCountFees;
        for (var i = 0; i < $('.fees_table').find('.gai').length; i++) {
            $('.fees_table').find('.gai').eq(i).val(Math.round($('.fees_table').find('.gai').eq(i).val() * 100 * discount) / 100)
            docCount += $('.fees_table').find('.gai').eq(i).val() * 100;
        }
        $('.basePicInput').val(parseInt($(this).val() * 100 - docCount) / 100)
        docCount = 0;
    })
 ...
 $('.layui-timeline').html(_html);
    ...
    $('.submitBtn').click(function() {
        ...
        for (var i = 0; i < $('.fees_table').find('.gai').length; i++) {
            basePriceArr.push({
                'doctorId': $('.fees_table').find('.gai').eq(i).attr('name'),
                'price': $('.fees_table').find('.gai').eq(i).val()
            })
        }
        $.ajax({
            type: 'POST',
            url: IP + 'order/updatePrice',
            dataType: 'json',
            data: {
                "orderId": data.orderFormBean.id,
                "totalPrice": $('.countInput').val(),
                "basePrice": $('.basePicInput').val(),
                "doctorPrice": JSON.stringify(basePriceArr),
                "type": 0,
            },
            xhrFields: {
                withCredentials: true
