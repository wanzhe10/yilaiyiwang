$(function () {
    ...
    //  校验年龄 身高 体重
    $('#age').blur(function () {
        if (!RegExpObj.Reg_Number.test($('#age').val())) {
            var _$ = layui.jquery;
            layer.open({
                type: 1,
                title: '',
    ...
    //    验证电话号码
    $('#phone').blur(function () {
        if (!RegExpObj.Reg_Number.test($('#phone').val())) {
            var _$ = layui.jquery;
            layer.open({
                type: 1,
                title: '',
                ...
                if (data.status == 200) {
                    ...
                    var _html = '<li class="doctorChunk noDocter">\
                        ...
                        <div class="message">\
                            ...
                            <p class="p1" name="' + hospitalId + '">' + hospitalName + '</p>\
                            <p class="p4">选择此项,申请将发送至对方医院远程中心,由医务人员为您调度医生资源,诊费会在选定医生后确定。<br />请將您的备注信息填至【会/转诊目的】 </p>\
                        </div>\
                    </li>';
                ...
                if (flag) {
                    ...
                    objParent.append(
                        `<li class="fileItem">\
                            ...
                            <img class="delFileBtn" src="/yilaiyiwang/images/delete_file.png"/><p type="${uploadFile[fileLength].type}" desc="" class="fileName">${uploadFile[fileLength].name}</p></li>`
                    );
                    // 总张数
                    // objParent.siblings('.operateLeft').find('.fileCount').html(objParent.find('.fileItem').length);
    ...
    //     console.log($('.upfileContent').find('.fileItem').children('div').css("background-image").split("\"")[1])
    //     // 获取到pdf的url地址
    //     var url = $('.upfileContent').find('.fileItem').children('div').css("background-image").split("\"")[1];
    ...
    $('.upfileUl').delegate('.fileItem', 'click', function () {
        /* PDF预览 */
        var url = $('.upfileContent').find('.fileItem').children('div').css("background-image").split("\"")[1];
        // console.log($('.upfileContent').find('.fileItem').children('.fileName').attr('type'))
        var nameSuffix = $('.upfileContent').find('.fileItem').children('.fileName').attr('type');
        console.log(nameSuffix);
        var suffixPdf = nameSuffix.split('/').pop();
        // console.log(suffixPdf === 'pdf')
        if (suffixPdf == 'pdf') {
            console.log($('.pdfmedia').attr('href'))
            var media1 = $('.pdfmedia').attr('href')
            media1 = url ;
            console.log(media1)
            $('.pdfmedia').css('display','block');
            // $('.pdfmedia').css('display', 'block');
            // showPdf()
        }else{
        ...
        // var data = document.getElementsByClassName("fileItem").getAttribute("type");
    ...
    $('.ServeDrafts').click(function () {
        ...
        data.append('isUrgent', $('.urgent > a.active').attr('value')); //是否加急(0是1不是)
        ...
        if (fileAllArr.length > 0) {
            // 原始文件数据 fileAllArr
            for (var i = 0; i < fileAllArr.length; i++) {
                data.append('file', fileAllArr[i].value)
            }
        ...
        if ($('#username').val() == '' && $('#idCard').val() == '' && $('#phone').val() == '' && $('#address').val() == '' && $('#age').val() == '' && $('#high').val() == '' && $('#weight').val() == '' && $('#createCase_textDiagnose').val() == '' && $('#createCase_textGola').val() == '') {
            layer.open({
                type: 1,
                title: '',
                area: ['300px', '80px'],
    ...
    $('.imgContent .noBtn').click(function () {
        $('.yesBtn').hide();
        $('.selectTimeContainer').hide();

    })
    $('.imgContent .yesBtn').click(function () {
        var data = new FormData();
        data.append('name', $('#username').val());
        data.append('idCard', $('#idCard').val());
        data.append('phone', $('#phone').val());
